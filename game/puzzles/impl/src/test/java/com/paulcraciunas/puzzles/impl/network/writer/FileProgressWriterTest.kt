...
internal class FileProgressWriterTest {
    ...
    private val underTest = FileProgressWriter(reporter)
    ...
    @Test
    fun `GIVEN empty input stream WHEN write is called THEN empty file is created`(@TempDir tempDir: File) = runTest {
        // Given
    ...
    @Test
    fun `GIVEN failing input stream WHEN write is called THEN exception is propagated`(@TempDir tempDir: File) = runTest {
        // Given
        val destination = File(tempDir, "test.txt")
    ...
    @Test
    fun `GIVEN large file WHEN write is called THEN progress is reported correctly`(@TempDir tempDir: File) = runTest {
        // Given
