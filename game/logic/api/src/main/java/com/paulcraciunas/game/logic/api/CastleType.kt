...
enum class CastleType(val passFile: File, val endFile: File, val rookFile: File, val extra: File?) {
    ...
    fun from(turn: Side): Locus = Locus.from(File.e, rank(turn))
    fun pass(turn: Side): Locus = Locus.from(passFile, rank(turn))
    fun extraPass(turn: Side): Locus? = extra?.let { Locus.from(it, rank(turn)) }
    fun end(turn: Side): Locus = Locus.from(endFile, rank(turn))
    fun rook(turn: Side): Locus = Locus.from(rookFile, rank(turn))
