...
class BinaryAdapter {
    ...
    fun toBinary(
        ...
        var result = 0
        if (white.contains(CastleType.KingSide)) result = result or 0b1
        if (white.contains(CastleType.QueenSide)) result = result or 0b10
        if (black.contains(CastleType.KingSide)) result = result or 0b100
        if (black.contains(CastleType.QueenSide)) result = result or 0b1000
        return result
    }
    ...
    fun toCastling(data: Int): Pair<Set<CastleType>, Set<CastleType>> {
        val white = HashSet<CastleType>()
        val black = HashSet<CastleType>()

        if ((data and 0b1) != 0) white.add(CastleType.KingSide)
        if ((data and 0b10) != 0) white.add(CastleType.QueenSide)
        if ((data and 0b100) != 0) black.add(CastleType.KingSide)
        if ((data and 0b1000) != 0) black.add(CastleType.QueenSide)
