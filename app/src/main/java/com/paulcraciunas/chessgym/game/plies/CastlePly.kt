...
class CastlePly(override val turn: Side, val type: Type) : Ply {
    ...
    enum class Type(val passFile: File, val endFile: File, val rookFile: File) {
        KingSide(passFile = File.f, endFile = File.g, rookFile = File.h),
        QueenSide(passFile = File.d, endFile = File.c, rookFile = File.a);
